...
const MultiSelectTrigger = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { placeholder?: string; maxDisplayItems?: number }>(
    ({ className, placeholder = "Select items...", maxDisplayItems = 3, ...props }, ref) => {
        const context = useSelectContext()
        const config = useFyskConfig()
        ...
        return (
            <Popover.Trigger asChild>
                <div
                    ...
                    tabIndex={0}
                    className={cn(
                        "flex min-h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all cursor-pointer",
                        className
                    )}
                    onKeyDown={(e) => {
                ...
                >
                        ...
                        {hasValues ? (
                            <>
                                {displayValues.map((value) => (
                                    <span key={value} className="inline-flex items-center gap-1 rounded-sm bg-secondary px-2 py-0.5 text-xs font-medium text-secondary-foreground">
                                        {value}
                                        <div
                                            role="button"
                                            tabIndex={0}
                    ...
                    <span className="h-4 w-4 shrink-0 opacity-50 ml-2 flex items-center justify-center [&_svg]:size-full">
                        {config.icons?.chevronDown}
                    </span>
                </div>
            </Popover.Trigger>
...
const SelectTrigger = React.forwardRef<
    ...
    SelectTriggerProps
>(({ className, children, placeholder, maxDisplayItems = 3, ...props }, ref) => {
    const context = useSelectContext()
    const config = useFyskConfig()
    ...
    return (
        ...
        >
            ...
            <SelectPrimitive.Icon asChild>
                <span className="h-4 w-4 opacity-50 flex items-center justify-center [&_svg]:size-full">
                    {config.icons?.chevronDown}
                </span>
